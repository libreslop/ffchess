//! Victory overlay component displayed after winning a match.

use crate::utils::is_mobile;
use common::types::Score;
use yew::prelude::*;

/// Properties for the victory overlay.
#[derive(Properties, PartialEq)]
pub struct VictoryScreenProps {
    pub title: String,
...
#[function_component(VictoryScreen)]
/// Renders the victory summary and rejoin button.
///
/// `props` supplies score stats, text, and callbacks. Returns rendered HTML.
pub fn victory_screen(props: &VictoryScreenProps) -> Html {
        ...
        <>
            ...
            <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 100; text-align: center; width: 90%; max-width: 400px; color: #fff;">
                <div style="animation: fadeIn 0.3s ease-out;">
                    <h1 style={format!("color: #22c55e; margin-top: 0; font-size: {}; letter-spacing: 4px; text-shadow: 3px 3px 0 rgba(0,0,0,0.6); text-transform: uppercase;", title_size)}>{props.title.clone()}</h1>

                    if let Some(message) = props.message.clone() {
                        if !message.trim().is_empty() {
                            <p style="margin: 0 0 20px 0; font-size: 0.95em; color: #bbf7d0; letter-spacing: 0.5px;">{message}</p>
                        }
                    }
