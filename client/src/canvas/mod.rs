...
impl Renderer {
    pub fn draw_with_ghosts(
        ...
        for x in start_x..end_x {
            for y in start_y..end_y {
                ...
                if (x.rem_euclid(2) + y.rem_euclid(2)) % 2 != 0 {
                    self.ctx.fill_rect(
                        x as f64 * self.tile_size + offset_x,
                        y as f64 * self.tile_size + offset_y,
                        self.tile_size,
                        self.tile_size,
                    );
                }
        ...
        for x in start_x..=end_x {
            self.ctx.move_to(
                x as f64 * self.tile_size + offset_x,
                start_y as f64 * self.tile_size + offset_y,
            );
            self.ctx.line_to(
                x as f64 * self.tile_size + offset_x,
                end_y as f64 * self.tile_size + offset_y,
            );
        }
        for y in start_y..=end_y {
            self.ctx.move_to(
                start_x as f64 * self.tile_size + offset_x,
                y as f64 * self.tile_size + offset_y,
            );
            self.ctx.line_to(
                end_x as f64 * self.tile_size + offset_x,
                y as f64 * self.tile_size + offset_y,
            );
        }
        self.ctx.stroke();
        ...
        for shop in &state.shops {
            if (shop.position - king_pos).abs().max_element() <= view_radius_squares + 2 {
                ...
                self.ctx.fill_rect(
                    shop.position.x as f64 * self.tile_size + offset_x + 5.0 * self.zoom,
                    shop.position.y as f64 * self.tile_size + offset_y + 5.0 * self.zoom,
                    self.tile_size - 10.0 * self.zoom,
                    self.tile_size - 10.0 * self.zoom,
                );
            }
        }
        ...
        {
            self.ctx
                .set_fill_style(&JsValue::from_str("rgba(34, 197, 94, 0.2)"));
            let range = 10;
            for x in (piece.position.x - range)..(piece.position.x + range + 1) {
                for y in (piece.position.y - range)..(piece.position.y + range + 1) {
                    ...
                    if is_valid_chess_move(
                        ...
                        let blocked = piece.piece_type != PieceType::Knight
                            && is_move_blocked(piece.position, t, &state.pieces);
                        if !blocked {
                            self.ctx.fill_rect(
                                x as f64 * self.tile_size + offset_x + 2.0,
                                y as f64 * self.tile_size + offset_y + 2.0,
                                self.tile_size - 4.0,
                                self.tile_size - 4.0,
                            );
                        }
        ...
        // Pmove lines
        self.ctx
            .set_stroke_style(&JsValue::from_str("rgba(59, 130, 246, 0.5)"));
        self.ctx.set_line_width(2.0);
        for pm in pm_queue {
            if let Some(real_p) = state.pieces.get(&pm.piece_id) {
                ...
                self.ctx.begin_path();
                self.ctx.move_to(
                    start_pos.x as f64 * self.tile_size + offset_x + self.tile_size / 2.0,
                    start_pos.y as f64 * self.tile_size + offset_y + self.tile_size / 2.0,
                );
                self.ctx.line_to(
                    pm.target.x as f64 * self.tile_size + offset_x + self.tile_size / 2.0,
                    pm.target.y as f64 * self.tile_size + offset_y + self.tile_size / 2.0,
                );
                self.ctx.stroke();
            }
        }
        ...
        if player_id != Uuid::nil() && has_king {
            let king_screen_x =
                king_pos.x as f64 * self.tile_size + offset_x + self.tile_size / 2.0;
            let king_screen_y =
                king_pos.y as f64 * self.tile_size + offset_y + self.tile_size / 2.0;
