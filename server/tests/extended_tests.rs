...
async fn test_idle_optimization() {
    ...
    state.player_channels.write().await.insert(Uuid::new_v4(), tx);
...
async fn test_shop_behavior_pawns() {
    let state = Arc::new(ServerState::new());
    let (tx, _rx) = mpsc::unbounded_channel();
    let player_id = state.add_player("Test".to_string(), KitType::Standard, tx, None).await;
    ...
    let initial_piece_count = {
        ...
        game.pieces.values().filter(|p| p.owner_id == Some(player_id)).count()
    };

    // Buy a pawn
    state.handle_shop_buy(player_id, IVec2::new(0, 0), PieceType::Pawn).await.expect("Should succeed");

    // Check that we have one more piece
    {
        let game = state.game.read().await;
        let current_count = game.pieces.values().filter(|p| p.owner_id == Some(player_id)).count();
        assert_eq!(current_count, initial_piece_count + 1);
        
        // Ensure the original piece is still there and still a Knight
        let piece_on_shop = game.pieces.values().find(|p| p.position == IVec2::new(0, 0)).unwrap();
...
async fn test_shop_behavior_upgrade() {
    ...
    let piece_id = Uuid::new_v4();
    {
        ...
        game.pieces.insert(piece_id, Piece {
            id: piece_id,
            owner_id: Some(player_id),
            piece_type: PieceType::Knight,
            position: IVec2::new(5, 5),
    ...
    state.handle_shop_buy(player_id, IVec2::new(5, 5), PieceType::Queen).await.expect("Should succeed");
...
async fn test_king_can_only_recruit_pawns() {
    ...
    
    {
        let game = state.game.read().await;
        let player = game.players.get(&player_id).unwrap();
        let piece_count = game.pieces.values().filter(|p| p.owner_id == Some(player_id)).count();
