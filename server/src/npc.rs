...
impl ServerState {
    ...
    pub async fn tick_npcs(&self) {
        ...
        for id in npc_ids {
            if moves_this_tick >= max_moves_per_tick { break; }
            ...
            if now > last_move + cd + 500 {
                let target = self.find_npc_target(id, pos, p_type).await;

                if let Some(t) = target {
                    let mut game = self.game.write().await;
                    let board_size = game.board_size;
                    
                    if !is_within_board(t, board_size) { continue; }
    ...
    async fn find_npc_target(&self, _id: Uuid, pos: IVec2, p_type: PieceType) -> Option<IVec2> {
        let game = self.game.read().await;
        let board_size = game.board_size;
        ...
        if let Some(pid) = visible_to_player {
            for piece in game.pieces.values() {
                if piece.owner_id == Some(pid)
                    && (piece.position - pos).abs().max_element() <= 12 {
                    player_nearby = true;
                    break;
                }
            }
        ...
        if player_nearby {
            ...
            if let Some(t) = king_target { return Some(t); }
            if let Some(t) = other_target { return Some(t); }
            ...
            for player in game.players.values() {
                if let Some(king) = game.pieces.get(&player.king_id) {
                    if !is_within_board(king.position, board_size) { continue; }
            ...
            if nearest_p_pos.is_none() {
                for other_p in game.pieces.values() {
                    if other_p.owner_id.is_some() {
                        if !is_within_board(other_p.position, board_size) { continue; }
            ...
            if let Some(npp) = nearest_p_pos {
                ...
                if best_move.is_some() { return best_move; }
            }
        }
        ...
        match p_type {
            PieceType::Pawn => {
                let mut rng = rand::thread_rng();
                let directions = [IVec2::new(0, -1), IVec2::new(1, 0), IVec2::new(0, 1), IVec2::new(-1, 0)];
    ...
    async fn spawn_npcs(&self) {
        ...
        let mut rng = rand::thread_rng();
        let current_npc_count = game.pieces.values().filter(|p| p.owner_id.is_none()).count();
